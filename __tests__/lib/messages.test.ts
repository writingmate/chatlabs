...
describe('parseCodeBlocksAndContent', () => {
    ...
    it('should parse a string with a single code block correctly', () => {
        ...
        expect(result).toEqual({
            ...
            codeBlocks: [
                { language: 'js', code: "console.log('Hello, world!');", messageId: "1", sequenceNo: 0 }
            ]
        });
    });
    ...
    it('should parse a string with multiple code blocks correctly', () => {
        ...
        const result = parseCodeBlocksAndContent(input, "1");
        expect(result).toEqual({
            parsedContent: "Text before code.\n[CODE_BLOCK_0]\nMore text.\n[CODE_BLOCK_1]",
            codeBlocks: [
                { language: 'python', code: "print('Hello')", messageId: "1", sequenceNo: 0 },
                { language: 'html', code: '<div>Hello</div>', messageId: "1", sequenceNo: 1 }
            ]
        });
    });
    ...
    it('should handle unfinished code blocks correctly', () => {
        const input = "Here is an unfinished code block:\n```js\nconsole.log('Hello, world!')";
        const result = parseCodeBlocksAndContent(input, "1");
        expect(result).toEqual({
