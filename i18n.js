...
const classnameDetector = {
  ...
  cacheUserLanguage(lng) {
    if (typeof document !== 'undefined') {
      const htmlElement = document.documentElement
      const classNames = htmlElement.className.split(' ').filter(c => !c.startsWith('lang-'))
...
i18n
    ...
    .init({
        ...
        detection: {
            order: ['classname', 'querystring', 'cookie', 'localStorage', 'navigator', 'htmlTag', 'path', 'subdomain'],
            caches: ['cookie'],
        },
        interpolation: {
