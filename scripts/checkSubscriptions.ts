...
async function getAllStripeSubscriptions(): Promise<StripeSubscription[]> {
  ...
  while (hasMore) {
    ...
    allSubscriptions = allSubscriptions.concat(subscriptions.data as StripeSubscription[]);
...
async function checkAndUpdateSubscriptions() {
  try {
    ...
    const stripeSubscriptionMap = new Map(stripeSubscriptions.map(sub => [sub.customer.id, sub]));
    const supabaseUserMap = new Map(supabaseUsers.map(user => [user.email, user]));
    ...
    for (const subscription of stripeSubscriptions) {
      ...
      if (!email || !stripePlan) {
        console.log(`Skipping subscription ${subscription.id} due to missing email or plan`);
        continue;
      }
      ...
      if (profileError) {
        console.log(`Error fetching profile for user ${email}: ${profileError.message}`);
        continue;
      }
      ...
      if (profile.stripe_customer_id !== stripeCustomerId) {
        ...
        console.log(`  Supabase stripe_customer_id: ${profile.stripe_customer_id}`);
      ...
      if (shouldUpdatePlan || shouldUpdateStripeCustomerId) {
        ...
        if (shouldUpdateStripeCustomerId) updateData.stripe_customer_id = stripeCustomerId;
    ...
    for (const profile of profiles) {
      if (profile.plan !== 'free' && profile.stripe_customer_id) {
        const stripeSubscription = stripeSubscriptionMap.get(profile.stripe_customer_id);
